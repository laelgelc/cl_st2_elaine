\begin{textsample}{POS Dim 2 – 2019 – Score 14.00 – 2019/t002063.txt}  \label{ex:f2_pos_037}
...
But we do now see them at scale, and what worries me most is just our ability to address it in a systemic way that is scalable, that has a rigorous understanding of how we ' re taking \textbf{action}, a transparent understanding of how we ' re taking \textbf{action} and a rigorous appeals process for when we ' re wrong, because we will be wrong.
...
And there ' s been data that ' s come out — Amnesty International put out a report a few months ago where they showed that a subset of active black female Twitter users were receiving, on average, one in 10 of their \textbf{tweets} were some form of harassment.
...
JD : Yeah.
So it ' s a pretty terrible situation when you ' re coming to a \textbf{service} that, ideally, you want to learn something about the world, and you spend the majority of your time reporting abuse, receiving abuse, receiving harassment.
So what we ' re looking most deeply at is just the incentives that the \textbf{platform} naturally provides and the \textbf{service} provides.
Right now, the dynamic of the system makes it super-easy to harass and to abuse others through the \textbf{service}, and unfortunately, the majority of our system in the past worked entirely based on people reporting harassment and abuse.
...
About 38 percent of abusive \textbf{tweets} are now proactively identified by machine learning algorithms so that people don ' t actually have to report them.
...
So we need to continue to build \textbf{empathy} for what people are experiencing and give them better tools to act on it and also give our customers a much better and easier approach to handle some of the things that they ' re seeing.
So a lot of what we ' re doing is around technology, but we ' re also looking at the incentives on the \textbf{service} : What does Twitter incentivize you to do when you first open it up?
...
And we have to look a lot deeper at some of the fundamentals of what the \textbf{service} is doing to make the bigger shifts.
...
JD : Well, one of the things — we started the \textbf{service} with this concept of following an account, as an example, and I don ' t believe that ' s why people actually come to Twitter.
...
But that is a huge fundamental shift to \textbf{bias} the entire network away from just an account \textbf{bias} towards a topics and interest \textbf{bias}.
...
Like, from the point of view of people who just read Twitter, that ' s not an issue, but for the people who actually create it, everyone ' s out there saying,"You know, I wish I had a few more ' likes, ' followers, retweets."And so they ' re constantly experimenting, trying to find the path to do that.
...
We decided that number should be big and \textbf{bold}, and anything that ' s on the page that ' s big and \textbf{bold} has importance, and those are the things that you want to drive.
Was that the right decision at the time?
Probably not.
If I had to start the \textbf{service} again, I would not emphasize the follower count as much.
I would not emphasize the"like"count as much.
I don ' t think I would even create"like"in the first place, because it doesn ' t actually push what we believe now to be the most important thing, which is healthy contribution back to the network and conversation to the network, participation within conversation, learning something from the conversation.
Those are not things that we thought of 13 years ago, and we believe are extremely important right now.
So we have to look at how we display the follower count, how we display retweet count, how we display"likes,"and just ask the deep question : Is this really the number that we want people to drive up?
Is this the thing that, when you open Twitter, you see,"That ' s the thing I need to increase?"And I don ' t believe that ' s the case right now.
WPR : I think we should look at some of the \textbf{tweets} that are coming in from the audience as well.
...
WPR : I think one I saw that passed already quickly down here,"What ' s Twitter ' s plan to combat foreign meddling in the 2020 US election?"I think that ' s something that ' s an issue we ' re seeing on the internet in general, that we have a lot of malicious automated activity happening.
...
We have a toxicity \textbf{score}, a toxicity model, on our system that can actually measure whether you are likely to walk away from a conversation that you ' re having on Twitter because you feel it ' s toxic, with some pretty high degree.
...
JD : So we have policies around violent extremist groups, and the majority of our work and our terms of \textbf{service} works on conduct, not content.
So we ' re actually looking for conduct.
Conduct being using the \textbf{service} to repeatedly or episodically harass someone, using hateful imagery that might be associated with the KKK or the American Nazi Party.
Those are all things that we act on immediately.
We ' re in a situation right now where that term is used fairly loosely, and we just cannot take any one mention of that word accusing someone else as a factual indication that they should be removed from the \textbf{platform}.
So a lot of our models are based around, number one : Is this account associated with a violent extremist group?
And if so, we can take \textbf{action}.
...
We want to have a situation where algorithms are constantly scouring every single \textbf{tweet} and bringing the most interesting ones to the top so that humans can bring their judgment to whether we should take \textbf{action} or not, based on our terms of \textbf{service}.
...
So when people — just as an example, if you go to our current terms of \textbf{service} and you bring the page up, and you ' re wondering about abuse and harassment that you just received and whether it was against our terms of \textbf{service} to report it, the first thing you see when you open that page is around intellectual property protection.
You \textbf{scroll} down and you get to abuse, harassment and everything else that you might be experiencing.
...
So we want to make sure that we ' re not just encouraging more work around something that ' s \textbf{super}, \textbf{super} negative, and we want to have a good balance between the technology and where humans can actually be creative, which is the judgment of the rules, and not just all the mechanical stuff of finding and reporting them.
...
JD : Well, first and foremost, my personal goal with the \textbf{service} is that I believe fundamentally that public conversation is critical.
...
JD : Yeah ; more relevance means less time on the \textbf{service}, and that ' s perfectly fine, because we want to make sure that, like, you ' re coming to Twitter, and you see something immediately that you learn from and that you push.
We can still serve an \textbf{ad} against that.
That doesn ' t mean you need to spend any more time to see more.
The second thing we ' re looking at — CA : But just — on that goal, \textbf{daily} active usage, if you ' re measuring that, that doesn ' t necessarily mean things that people value every day.
It may well mean things that people are drawn to like a moth to the flame, every day.
We are addicted, because we see something that pisses us off, so we go in and add fuel to the fire, and the \textbf{daily} active usage goes up, and there ' s more \textbf{ad} revenue there, but we all get angrier with each other.
How do you define..."\textbf{Daily} active usage"seems like a really dangerous term to be optimizing.
...
Ultimately, we want to get to a metric where people can tell us,"Hey, I learned something from Twitter, and I ' m walking away with something valuable."That is our goal ultimately over time, but that ' s going to take some time.
...
It ' s not as big as some of the other \textbf{platforms}, but the people of influence use it to set the agenda, and it ' s just hard to imagine a more important role in the world than to...
...
One, when I came back to the company, we were in a pretty dire state in terms of our future, and not just from how people were using the \textbf{platform}, but from a corporate narrative as well.
...
It took courage.
I really appreciate what you said, and good luck with your \textbf{mission}.
JD : Thank you so much.
Thanks for having me.
Thank you.

% matched lemmas: action, ad, bias, bold, daily, empathy, maximize, mission, platform, score, scroll, service, super, tweet
\end{textsample}
