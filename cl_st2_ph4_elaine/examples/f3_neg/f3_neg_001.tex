\begin{textsample}{NEG Dim 3 – 1994 – Score -29.00 – 1994/t002458.txt}  \label{ex:f3_neg_001}
...
This happens to be a slide of some analysis that we were doing about the power of RISC \textbf{microprocessors} versus the power of local area networks.
...
So if transportation technology was moving along as fast as \textbf{microprocessor} technology, then the day after tomorrow, I would be able to get in a taxi cab and be in Tokyo in 30 seconds.
...
We can all sense it.
And we know that it just doesn ' t make too much sense to think out 30, 50 years because everything ' s going to be so different that a simple \textbf{extrapolation} of what we ' re doing just doesn ' t make any sense at all.
...
So if you go back about two and a half billion years, the Earth was this big, \textbf{sterile} \textbf{hunk} of rock with a lot of chemicals floating around on it.
...
Now that ' s sort of just a very simple chemical form of life, but when things \textbf{got} interesting was when these drops learned a trick about abstraction.
...
So in other words, they worked out, in this \textbf{mindless} sort of evolutionary way, a form of writing that let them write down what they were, so that that way of writing it down could get copied.
...
In fact, one of the things that I did just for \textbf{amusement} purposes is we can now write things in this code.
And I ' ve got here a little 100 \textbf{micrograms} of white powder, which I try not to let the security people see at airports.
But this has in it — what I did is I took this code — the code has standard letters that we use for \textbf{symbolizing} it — and I wrote my business card onto a piece of DNA and amplified it 10 to the 22 times.
...
Some bacteria figured out how to stay away from \textbf{penicillin}, and it went around sort of creating its little DNA information with other bacteria, and now we have a lot of bacteria that are resistant to \textbf{penicillin}, because bacteria communicate.
Now what this communication allowed was communities to form that, in some sense, were in the same boat together ; they were \textbf{synergistic}.
...
And at that stage, we have \textbf{multi-cellular} communities, communities of lots of different types of cells, working together as a single organism.
...
So neurons are the information processing \textbf{apparatus} that those communities of cells built up.
...
I ' m sitting here making \textbf{grunting} sounds basically, and hopefully constructing a similar messy, confused idea in your head that bears some analogy to it.
...
We ' re going through the same levels that \textbf{multi-cellular} organisms have gone through — abstracting out our methods of recording, presenting, processing information.
So for example, the invention of language was a tiny step in that direction. \textbf{Telephony}, computers, \textbf{videotapes}, CD-ROMs and so on are all our specialized mechanisms that we ' ve now built within our society for handling that information.
...
So now, evolution can take place on a scale of \textbf{microseconds}.
And you saw Ty ' s little evolutionary example where he sort of did a little bit of evolution on the \textbf{Convolution} program right before your eyes.
...
I don ' t know what every \textbf{transistor} in the connection machine does.
...
So what we do is we put inside the machine a process of evolution that takes place on the \textbf{microsecond} time scale.
...
And let ' s reproduce them by a process of \textbf{recombination} \textbf{analogous} to sex."Take two programs and they produce children by exchanging their \textbf{subroutines}, and the children inherit the traits of the \textbf{subroutines} of the two programs.
...
Well every one of those generations just takes a few \textbf{milliseconds}.
...
We ' re taking off.
And what we are is we ' re at a point in time which is \textbf{analogous} to when single- celled organisms were turning into multi-celled organisms.
So we ' re the \textbf{amoebas} and we can ' t quite figure out what the hell this thing is we ' re creating.
We ' re right at that point of transition.
But I think that there really is something coming along after us.
I think it ' s very \textbf{haughty} of us to think that we ' re the end product of evolution.
...
% matched lemmas: amoeba, amusement, analogous, apparatus, convolution, extrapolation, got, grunt, haughty, hunk, microgram, microprocessor, microsecond, millisecond, mindless, multi-cellular, penicillin, recombination, sterile, subroutine, symbolize, synergistic, telephony, transistor, videotape
\end{textsample}
