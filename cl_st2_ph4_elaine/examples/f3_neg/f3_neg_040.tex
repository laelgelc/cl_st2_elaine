\begin{textsample}{NEG Dim 3 – 2007 – Score -2.00 – 2007/t000230.txt}  \label{ex:f3_neg_040}
...
This is a quote, actually, from Alan Turing :"In 30 years, it will be as easy to ask a computer a question as to ask a person."This was in 1946.
...
Just to go back to the States, if you translate the amount of power or electricity this computer uses to the amount of households in the States, you get 1, 200 households in the U.
S.
...
So, this clip is from the PBS series,"The Secret Life of the Brain."It shows you these cells that process information.
...
This is a quote from Brian Eno :"the problem with computers is that there is not enough Africa in them."Brian actually said this in 1995.
...
The devices that computers use are what ' s called a \textbf{transistor}.
...
Now, what ' s happening is that as \textbf{transistors} are getting smaller and smaller and smaller, they no longer behave like this.
...
And these are individual \textbf{potassium} ions that are flowing through that pore.
...
Now, this is starting to happen in \textbf{transistors}.
And the reason why that ' s happening is that, right now, in 2007 — the technology that we are using — a \textbf{transistor} is big enough that several electrons can flow through the channel simultaneously, side by side.
In fact, there ' s about 12 electrons can all be flowing this way.
And that means that a \textbf{transistor} corresponds to about 12 ion channels in parallel.
Now, in a few years time, by 2015, we will shrink \textbf{transistors} so much.
This is what Intel does to keep adding more cores onto the chip.
Or your memory sticks that you have now can carry one gigabyte of stuff on them — before, it was 256. \textbf{Transistors} are getting smaller to allow this to happen, and technology has really benefitted from that.
But what ' s happening now is that in 2015, the \textbf{transistor} is going to become so small, that it corresponds to only one electron at a time can flow through that channel, and that corresponds to a single ion channel.
...
So now the neurons are represented by little nodes or circuits on the chip, and the connections among the neurons are represented, actually modeled by \textbf{transistors}.
And these \textbf{transistors} are behaving essentially just like ion channels behave in the brain.
...
We can dream like everybody else.
CA : Thanks Kwabena, that was really interesting.
Thank you.

% matched lemmas: potassium, transistor
\end{textsample}
