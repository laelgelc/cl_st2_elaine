\begin{textsample}{POS Dim 4 – 1994 – Score 6.00 – 1994/t002458.txt}  \label{ex:f4_pos_067}
...
And the \textbf{interesting} thing about it is that this slide, like so many technology slides that we ' re used to, is a sort of a straight line on a semi-log curve.
...
Now if I graph, for instance, some other technology, say transportation technology, on a semi-log curve, it would look very \textbf{stupid}, it would look like a flat line.
...
If so, that means that what we ' re in the middle of right now is a \textbf{transition}.
We ' re sort of on this line in a \textbf{transition} from the way the world used to be to some new way that the world is.
...
Because the \textbf{transition} seems very, very confusing when we ' re right in the middle of it.
...
Now here ' s a \textbf{conference} in which people talk about the future, and you notice that the future is still at about the year 2000.
...
That \textbf{transition} is happening.
We can all sense it.
...
So what I would like to talk about is what that could be, what that \textbf{transition} could be that we ' re going through.
Now in order to do that I ' m going to have to talk about a bunch of \textbf{stuff} that really has nothing to do with technology and computers.
...
Now that ' s sort of just a very simple chemical form of life, but when things got \textbf{interesting} was when these drops learned a trick about abstraction.
...
If I had really been a egotist, I would have put it into a \textbf{virus} and released it in the room.
So what was the next step?
Writing down the DNA was an \textbf{interesting} step.
And that caused these cells — that kept them happy for another billion years.
But then there was another really \textbf{interesting} step where things became completely different, which is these cells started exchanging and communicating information, so that they began to get communities of cells.
...
Now the \textbf{transition} point happened when these communities got so close that, in fact, they got together and decided to write down the whole recipe for the community together on one string of DNA.
And so the next stage that ' s \textbf{interesting} in life took about another billion years.
...
So these communities began to evolve so that the \textbf{interesting} level on which evolution was taking place was no longer a cell, but a community which we call an organism.
...
One method that ' s particularly \textbf{interesting} that I ' ve been using a lot lately is evolution itself.
...
Look at this.
It says, ' This plane has hundreds of thousands of tiny parts working together to make you a \textbf{safe} flight. ' Doesn ' t that make you feel confident?"In fact, we know that the engineering process doesn ' t work very well when it gets complicated.
...
We ' re right at that point of \textbf{transition}.
...
% matched lemmas: conference, interesting, safe, stuff, stupid, transition, virus
\end{textsample}
