\begin{textsample}{POS Dim 6 – 2015 – Score 4.00 – 2015/t001423.txt}  \label{ex:f6_pos_052}
...
And so designers have become very adept at creating \textbf{interfaces} that allow you to manipulate parameters while you ' re attending to other things, such as taking a photograph and changing the focus or the aperture.
...
However, computers have the same static physical form for all of these different applications and the same static \textbf{interface} elements as well.
...
And my belief is that, then, we must need new types of \textbf{interfaces} that can capture these rich abilities that we have and that can physically adapt to us and allow us to interact in new ways.
And so that ' s what I ' ve been doing at the MIT Media Lab and now at Stanford.
So with my colleagues, Daniel Leithinger and Hiroshi Ishii, we created inFORM, where the \textbf{interface} can actually come off the screen and you can physically manipulate it.
...
Or \textbf{interface} elements can arise out of the surface and change on demand.
...
So with Tony Tang at the Media Lab, we created an \textbf{interface} built on inFORM to allow urban planners to design and view entire cities.
...
We also believe that these dynamic shape \textbf{displays} can really change the ways that we remotely collaborate with people.
...
So we used the pins of the \textbf{display} to represent people ' s hands, allowing them to actually touch and manipulate objects at a distance.
...
Or you can have an object that ' s linked between two places, so as I move a \textbf{ball} on one side, the \textbf{ball} moves on the other as well.
...
So we sort of set up a mini-sweatshop in the Media Lab and brought undergrads in and convinced them to do"research"— and had late nights watching \textbf{movies}, eating pizza and screwing in thousands of screws.
You know — research.
...
So at Stanford, we created this haptic edge \textbf{display}, which is a mobile device with an array of linear actuators that can change shape, so you can feel in your hand where you are as you ' re reading a book.
...
And we ' re also interested in looking at ways that users can actually deform the \textbf{interfaces} to shape them into the devices that they want to use.
...
So for the Milan Design Week, we created TRANSFORM, which is an interactive table-scale version of these shape \textbf{displays}, which can move physical objects on the surface ; for example, reminding you to take your keys.
...
% matched lemmas: ball, display, interface, movie
\end{textsample}
