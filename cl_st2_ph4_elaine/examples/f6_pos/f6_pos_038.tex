\begin{textsample}{POS Dim 6 – 2010 – Score 8.00 – 2010/t002842.txt}  \label{ex:f6_pos_038}
We ' re 25, 26 years after the advent of the Macintosh, which was an astoundingly seminal event in the history of human-machine \textbf{interface} and in computation in general.
...
Now, this is an interesting little message, and it ' s a lesson that has since, I think, been forgotten or lost or something, and that is, namely, that the OS is the \textbf{interface}.
The \textbf{interface} is the OS.
...
Then, on the side, we ' ve got the Web and, increasingly, the cloud, which is fantastic, but also — in the regard in which an \textbf{interface} is fundamental — kind of a distraction.
So we ' ve forgotten to invent new \textbf{interfaces}.
...
The problem, as we see it, has to do with a single, simple word :"space,"or a single, simple phrase :"real world geometry."Computers and the programming languages that we talk to them in, that we teach them in, are hideously insensate when it comes to space.
...
It ' s not even on the same plane as what you ' re talking about : The pixels are up on the \textbf{display}.
So here was a room in which all the walls, floors, ceilings, pets, potted plants, whatever was in there, were capable, not only of \textbf{display} but of sensing as well.
...
So you ' ve got an \textbf{interface} that has no \textbf{interface}.
You operate the world as you operate the real world, which is to say, with your hands.
Similarly, a digital wind tunnel with digital wind flowing from right to left — not that \textbf{remarkable} in a sense ; we didn ' t invent the mathematics.
But if you \textbf{displayed} that on a CRT or flat panel \textbf{display}, it would be meaningless to hold up an arbitrary object, a real world object in that.
...
But Alex was preparing a little, sort of obscure, indie, arthouse film called"Minority Report"for Steven Spielberg, and invited us to come out from MIT and design the \textbf{interfaces} that would appear in that film.
...
So this led full circle, in a strange way, to build these ideas into what we believe is the necessary future of human machine \textbf{interface} : the \textbf{Spatial} Operating Environment, we call it.
...
And it ' s fun to fly through Mr.
Beckett ' s eye.
...
Much of what we want computers to help us with in the first place is inherently \textbf{spatial}.
And the part that isn ' t \textbf{spatial} can often be spatialized to allow our wetware to make greater sense of it.
...
In the real world, we have people who collaborate ; we have people who have to work together, and we have many different \textbf{displays}.
And we might want to look at these \textbf{various} images.
...
So we have a large number of \textbf{movies} stuck inside here.
...
Can it be for every man, or is this just for corporations and \textbf{movie} producers?
...
Can there really be only one \textbf{interface}?
There can ' t.
CA : But does that mean that, at your desk or in your home, you need projectors, cameras?
You know, how can it work?
JU : No, this stuff will be built into the bezel of every \textbf{display}.
It ' ll be built into architecture.
The gloves go away in a matter of months or years.
So this is the inevitability about it.
CA : So, in your mind, five years time, someone can buy this as part of a standard computer \textbf{interface}?
...
% matched lemmas: display, interface, movie, remarkable, spatial, various
\end{textsample}
